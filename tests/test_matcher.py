...
def test_find_best_match_invalid_input(matcher, invalid_input):
    """测试 find_best_match 处理 None 或空字符串"""
    matches = matcher.find_best_match(invalid_input, version="csco22")
    assert matches == [], f"❌ 预期 {invalid_input} 返回空列表，但得到 {matches}"